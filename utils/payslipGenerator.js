...
// Concurrency Semaphore configurations to prevent memory/CPU spikes
...
/**
 * Helper to escape HTML to prevent XSS / local file reads via PDF injection
 * @param {string} unsafe
 * @returns {string}
 */
...
/**
 * Lazily initializes or retrieves the singleton Puppeteer browser instance
 */
 ...
 * Enforces maximum concurrency queue to protect against DoS.
 * 
...
const generatePayslipPdf = async (companyName, employeeRecord, periodName) => {
  ...
  try {
    ...
    const escapedStaffId = escapeHtml(staffId);
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Payslip - ${escapedName}</title>
        <style>
          body {
        ...
        <div class="container">
          <div class="header">
            <div class="company-info">
              <h1>${escapedCompanyName}</h1>
              <p>HR & Payroll Division</p>
            </div>
            <div class="payslip-title">
              <h2>PAYSLIP</h2>
              <p>${escapedPeriodName}</p>
            </div>
          </div>
          ...
          <div class="meta-grid">
            <div class="meta-item">
              ...
              <span class="meta-value">${escapedName}</span>
            </div>
            <div class="meta-item">
              <span class="meta-label">Staff ID:</span>
              <span class="meta-value">${escapedStaffId}</span>
            </div>
            <div class="meta-item">
              <span class="meta-label">Pay Period:</span>
              <span class="meta-value">${escapedPeriodName}</span>
            </div>
            <div class="meta-item">
    ...
    let page;
    try {
      browser = await getBrowser(puppeteer);
